...
def send_request():
    ...
    rcv_rqt = session.query(Contact).filter(and_(user_id=receiver.id, contact_id=sender.id)).one_or_none()
    rcv_rqt.status = Status.requested
    send_rqt = session.query(Contact).filter(and_(user_id=sender.id, contact_id=receiver.id)).one_or_none()
...
def block_contact():
    ...
    rqt = session.query(Contact).filter(and_(user_id=sender.id, contact_id=receiver.id)).one_or_none()
